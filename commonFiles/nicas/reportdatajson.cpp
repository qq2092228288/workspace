...
QString ReportDataJson::getReportTime()
{
    ...
    if (array.size() != 0) {
        return array.last().toObject().value(ReportDataName::ekey(ReportDataName::reportTime)).toString();
    }
    return QString();
}
...
void ReportDataJson::setPatientInfo(const QString &name, const QString &id, int gender, int age, int height, int weight)
{
    clear();
    json.insert(ReportDataName::ekey(ReportDataName::startTime), time());
    json.insert(ReportDataName::ekey(ReportDataName::patientInfo), QJsonObject
                {
                    ...
                    { ReportDataName::ekey(ReportDataName::height), QString::number(height) },
                    { ReportDataName::ekey(ReportDataName::weight), QString::number(weight) },
                    { ReportDataName::ekey(ReportDataName::sex), (gender == 0 ? QString("男") : QString("女")) },
                    { ReportDataName::ekey(ReportDataName::patientName), name },
                    { ReportDataName::ekey(ReportDataName::medicalRecordNumber), id }
                });
}
